...
- **MCP over HTTP** — `http://127.0.0.1:5179/mcp` (streamable HTTP transport). Registered automatically when the Claude Code plugin is installed (Phase 4b) — no manual `claude mcp add` step needed from inside a repo that has the plugin. Current tools: `health_check`, `version`, `create_prompt`, `create_prompt_version`, `get_prompt`, `get_prompt_version_content`, `list_prompts`, `activate_prompt_version`, `get_promotion_policy`, `update_promotion_policy`, `submit_human_evaluation`, `get_human_evaluations`, `run_ai_evaluation`, `get_ai_evaluation_policy`, `update_ai_evaluation_policy`, `recommend_prompt`, `get_statistics`.
